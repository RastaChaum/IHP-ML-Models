...
# Check 1: Vérifier que gh CLI est installé
...
for doc in "${DOCS[@]}"; do
    if [ -f "$doc" ]; then
        ...
        check_error "$(basename $doc) manquant"
    fi
done
...
# Check 8: Vérifier pyproject.toml
print_header "8. Version (pyproject.toml)"

if [ -f "pyproject.toml" ]; then
    check_ok "pyproject.toml existe"
    
    VERSION=$(grep -m 1 'version = ' pyproject.toml | cut -d'"' -f2)
    if [ -n "$VERSION" ]; then
        check_ok "Version détectée: $VERSION"
    else
        check_error "Version non trouvée dans pyproject.toml"
    fi
else
    check_error "pyproject.toml manquant"
fi
...
if [ "$RELEASES_COUNT" -gt "0" ]; then
    ...
    check_info "Aucune release trouvée (normal pour nouveau projet)"
fi
