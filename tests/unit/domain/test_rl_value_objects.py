...
class TestRLObservation:
    ...
    def create_valid_observation(self, **kwargs) -> RLObservation:
        ...
        defaults = {
            "indoor_temp": 20.0,
            "indoor_temp_entity": EntityState("sensor.indoor_temp", 0.0),
            "outdoor_temp": 5.0,
            ...
            "heating_output_entity": EntityState("sensor.heating_output", 0.0),
            "energy_consumption_recent_kwh": 0.5,
            "energy_consumption_entity": EntityState("sensor.energy", 0.0),
            "time_heating_on_recent_seconds": 300,
            "time_heating_on_entity": EntityState("sensor.heating_time", 0.0),
    ...
    def test_indoor_temp_out_of_range_raises_error(self):
        """Test that indoor temperature out of range raises ValueError."""
        with pytest.raises(ValueError, match="indoor_temp must be between -25 and 50"):
            self.create_valid_observation(indoor_temp=-30.0)

        with pytest.raises(ValueError, match="indoor_temp must be between -25 and 50"):
            self.create_valid_observation(indoor_temp=60.0)

    def test_indoor_humidity_out_of_range_raises_error(self):
        """Test that indoor humidity out of range raises ValueError."""
        with pytest.raises(ValueError, match="indoor_humidity must be between 0 and 100"):
            self.create_valid_observation(indoor_humidity=-10.0)

        with pytest.raises(ValueError, match="indoor_humidity must be between 0 and 100"):
            self.create_valid_observation(indoor_humidity=110.0)
    ...
    def test_minimal_required_observation(self):
        """Test creating observation with only minimal required fields."""
        obs = RLObservation(
            indoor_temp=20.0,
            indoor_temp_entity=EntityState("sensor.indoor_temp", 0.0),
            outdoor_temp=None,
    ...
    def test_optional_fields_can_be_none(self):
        ...
        assert obs.indoor_humidity is None
...
class TestTrainingRequest:
    ...
    def test_valid_training_request(self):
        ...
        request = TrainingRequest(
            ...
            heating_state_entity_id="binary_sensor.heating_state",
            start_time=start,
            end_time=end,
    ...
    def test_start_time_none_is_allowed(self):
        """Test that start_time can be None."""
        request = TrainingRequest(
            device_id="ihp_salon",
            indoor_temp_entity_id="sensor.indoor_temp",
            target_temp_entity_id="sensor.target_temp",
    ...
    def test_optional_entity_ids_can_be_none(self):
        ...
        request = TrainingRequest(
            ...
            target_temp_entity_id="sensor.target_temp",
            heating_state_entity_id="binary_sensor.heating_state",
            outdoor_temp_entity_id=None,
            indoor_humidity_entity_id=None,
            window_or_door_open_entity_id=None,
            heating_power_entity_id=None,
        )

        assert request.outdoor_temp_entity_id is None
        assert request.indoor_humidity_entity_id is None
